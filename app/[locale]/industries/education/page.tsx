...
export default function EducationIndustryPage() {
  const t = useTranslations("industries.education.page");
  const [isDemoModalOpen, setIsDemoModalOpen] = useState(false);
  const [openFaqIndex, setOpenFaqIndex] = useState<number | null>(0);
  const [studentAssets, setStudentAssets] = useState([
    { id: "CB-2026-104", name: t("hero.cockpit.assets.chromebook300e"), student: t("hero.cockpit.students.liam"), tag: t("hero.cockpit.tags.cb104"), status: t("hero.cockpit.status.assigned"), condition: t("hero.cockpit.conditions.good") },
    { id: "CB-2026-219", name: t("hero.cockpit.assets.chromebook3110"), student: t("hero.cockpit.students.emma"), tag: t("hero.cockpit.tags.cb219"), status: t("hero.cockpit.status.assigned"), condition: t("hero.cockpit.conditions.pristine") },
    { id: "LB-2026-004", name: t("hero.cockpit.assets.labquest"), student: t("hero.cockpit.students.physicsLab"), tag: t("hero.cockpit.tags.lb004"), status: t("hero.cockpit.status.inLab"), condition: t("hero.cockpit.conditions.calibrated") },
  ]);
            ...
            <div className="grid lg:grid-cols-12 gap-12 lg:gap-10 items-center">
              ...
              <div className="lg:col-span-6 space-y-6 text-center lg:text-left">
                ...
                <div className="pt-2 flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3.5">
                  ...
                  >
                    <span>{t("hero.cta.exploreFeatures")}</span>
                  </Button>
                </div>
                    ...
                    {studentAssets.map((asset) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            ...
                            <div className="text-[11px] font-mono text-[var(--foreground-subtle)]">
                              {t("hero.cockpit.tag")} {asset.tag} &bull; {t("hero.cockpit.assignedTo")} <strong className="text-[var(--foreground)]">{asset.student}</strong>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center justify-between text-[11px] text-[var(--foreground-muted)] pt-1 border-t border-[var(--border-subtle)]">
                          <span>{t("hero.cockpit.condition")} <strong>{asset.condition}</strong></span>
                          <span className="text-[var(--brand-primary)] font-semibold">{t("hero.cockpit.readyForReturn")}</span>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="p-6 rounded-xl border border-[var(--border-custom)] bg-[var(--surface)] space-y-3">
                ...
                <h3 className="text-lg font-bold text-[var(--foreground)]">{t("capabilities.studentDevices.title")}</h3>
                <p className="text-sm text-[var(--foreground-muted)] leading-relaxed">
                  {t("capabilities.studentDevices.description")}
                </p>
              </div>
              ...
              <div className="p-6 rounded-xl border border-[var(--border-custom)] bg-[var(--surface)] space-y-3">
                ...
                <h3 className="text-lg font-bold text-[var(--foreground)]">{t("capabilities.stemLab.title")}</h3>
                <p className="text-sm text-[var(--foreground-muted)] leading-relaxed">
                  {t("capabilities.stemLab.description")}
                </p>
              </div>
              ...
              <div className="p-6 rounded-xl border border-[var(--border-custom)] bg-[var(--surface)] space-y-3">
                ...
                <h3 className="text-lg font-bold text-[var(--foreground)]">{t("capabilities.grantAudit.title")}</h3>
                <p className="text-sm text-[var(--foreground-muted)] leading-relaxed">
                  {t("capabilities.grantAudit.description")}
                </p>
              </div>
            </div>
          </div>
