        ...
        <div className="rounded-lg sm:rounded-xl border border-[var(--border-custom)] bg-[var(--surface)] p-4 sm:p-6 lg:p-8 xl:p-10 shadow-[var(--shadow-card)]">
          <div className="grid lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-10 items-center">
            ...
            <div className="lg:col-span-6 space-y-3 sm:space-y-4">
              <div className="rounded-lg sm:rounded-xl border border-[var(--border-custom)] bg-white p-3 sm:p-5 lg:p-6 shadow-sm space-y-3 sm:space-y-4">
                <div className="flex items-center justify-between pb-2 sm:pb-3 border-b border-[var(--border-subtle)] gap-2">
                  <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                    <div className="p-1 sm:p-1.5 rounded bg-[var(--brand-primary-light)] text-[var(--brand-primary)] shrink-0">
                      {currentInd.icon}
                    </div>
                    <div className="min-w-0">
                      <div className="text-[10px] sm:text-xs font-bold text-[var(--foreground)] truncate">
                  ...
                  <span className="text-[9px] sm:text-[11px] font-bold text-[var(--brand-emerald)] bg-[var(--brand-emerald-light)] px-1.5 sm:px-2 py-0.5 rounded border border-[var(--brand-emerald)]/20 shrink-0 whitespace-nowrap">
                ...
                <div className="space-y-1.5 sm:space-y-2">
                  {currentInd.mockupData.items.map((item, idx) => (
                    <div
                      key={idx}
                      className="p-2 sm:p-3 rounded-lg border border-[var(--border-custom)] bg-[var(--surface)] hover:bg-white transition-all text-[10px] sm:text-xs space-y-1"
                    >
                      <div className="flex items-start justify-between gap-1.5 sm:gap-2">
                        <div className="min-w-0">
                          <div className="font-bold text-[var(--foreground)] line-clamp-1">{item.name}</div>
                          <div className="text-[9px] sm:text-[11px] font-mono text-[var(--foreground-subtle)] line-clamp-1">
                            <span className="hidden sm:inline">TAG: {item.tag} &bull; Custody: </span>
                            <span className="sm:hidden">{item.tag} &bull; </span>
                            <strong className="text-[var(--foreground)]">{item.custodian}</strong>
                          </div>
                        </div>
                        <span className="px-1.5 sm:px-2 py-0.5 rounded text-[9px] sm:text-[10px] font-bold bg-[var(--brand-emerald-light)] text-[var(--brand-emerald)] border border-[var(--brand-emerald)]/20 shrink-0 whitespace-nowrap">
                          {item.status}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-[9px] sm:text-[11px] text-[var(--foreground-muted)] pt-0.5 sm:pt-1 border-t border-[var(--border-subtle)]">
                        <span className="truncate"><span className="hidden sm:inline">Details: </span>{item.value}</span>
                        <span className="text-[var(--brand-primary)] font-semibold shrink-0 ml-2 hidden sm:inline">Active Tag</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="relative h-20 sm:h-28 w-full rounded-lg overflow-hidden border border-[var(--border-custom)] group">
                  <Image
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-slate-900/40 to-transparent flex items-end p-2 sm:p-3">
